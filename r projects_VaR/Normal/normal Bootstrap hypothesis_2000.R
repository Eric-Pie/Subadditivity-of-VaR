...
cop_norm <- ellipCopula("normal", param=iTau(ellipCopula("normal"), tau=tau), dim=5)
original_U <- rCopula(n.obs, cop_norm)
X_5 <- apply(original_U, 2, qnorm)

## d=10
set.seed(121)
## generate a sample from a normal distribution
cop_norm <- ellipCopula("normal", param=iTau(ellipCopula("normal"), tau=tau), dim=10)
original_U <- rCopula(n.obs, cop_norm)
X_10 <- apply(original_U, 2, qnorm)

## d=20
set.seed(122)
## generate a sample from a normal distribution
cop_norm <- ellipCopula("normal", param=iTau(ellipCopula("normal"), tau=tau), dim=20)
original_U <- rCopula(n.obs, cop_norm)
X_20 <- apply(original_U, 2, qnorm)

### 2 Normal Non-para Bootstrap ###########################################
## Normal Non-para Bootstrap
norm_boot <- function(X, N.sim){
  n <- nrow(X)
  d <- ncol(X)
  norm_boot <- list(NA, N.sim)
  for(i in 1:N.sim){
...
Bootstrap_samples_5 <- norm_boot(X_5, N.sim)
Bootstrap_samples_10 <- norm_boot(X_10, N.sim)
Bootstrap_samples_20 <- norm_boot(X_20, N.sim)
...
pdf("normal Hypothesis_bootstrap_graph_n.obs=2000.pdf")
