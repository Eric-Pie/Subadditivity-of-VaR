...
### 1 Generate Data ############################################################
set.seed(123)
## generate a sample from a t-distribution with degree of freedom 4, Kendall's tau 0.5, dimension 20 and t copula.
...
t_boot <- function(X, N.sim){
  ...
  for(i in 1:N.sim){
    ## each row in X is a sample with dimiension 20, we resample n rows from X with replacement to generate a new sample space.
...
matplot((1-10^(seq(log10(1-0.8), log10(1-0.999), length.out = 100))), Matrix_bounds, type = "l", lty = 1, col = c("blue", "red","brown"), xlab = expression(alpha), ylab = substitute(Bootstrap~CI~of~F[S](d*{F^{-1}}(alpha))~"for"~l~alpha~"in [0.8,1]"~(d~"="~20), list(l = length(alpha))),xlim = c(0.8,1),ylim = c(0.8,1))
