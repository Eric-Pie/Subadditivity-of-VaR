...
n_Times <- 500 #the number of samples for N_star
n_res <- c()
library(simsalapar)
require(copula)
...
print(n_res)
## the sample mean and sample sd. of N_star
n_mean <- mean(n_res)
n_sd <- sd(n_res)

## r = 0.05, p = 0.9 calculate the n_sfc of N_star
## To show the full credibility of N_star sample mean as an estimator of E(N_star)
n_sfc = lambda*(n_sd/n_mean)^2
n_sfc
## but the n_sfc varies with different sample space. How can I confirm the result?
