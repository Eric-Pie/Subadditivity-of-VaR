...
### 1 Generate Data ############################################################
set.seed(123)
## generate a sample from a t-distribution with degree of freedom 4, Kendall's tau 0.5, dimension 100 and t copula.
cop_t <- ellipCopula("t", param=iTau(ellipCopula("t"), tau=tau), dim=d)
original_U <- rCopula(n.obs, cop_t)
qmargin_t4 <- function(p){
  qt(p, df=4)
}
X <- apply(original_U, 2, qmargin_t4) ## apply the quantile function to each column of original_U to get the base sample space
...
t_boot <- function(X, N.sim){
  ...
  for(i in 1:N.sim){
    ## each row in X is a sample with dimiension 100, we resample n rows from X with replacement to generate a new sample space.
...
matplot((1-10^(seq(log10(1-0.8), log10(1-0.999), length.out = 100))), Matrix_bounds, type = "l", lty = 1, col = c("blue", "red","brown"), xlab = expression(alpha), ylab = substitute(Bootstrap~CI~of~F[S](d*{F^{-1}}(alpha))~"for"~l~alpha~"in [0.8,1]"~(d~"="~100), list(l = length(alpha))),xlim = c(0.8,1),ylim = c(0.8,1))
